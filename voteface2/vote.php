...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['aadhar'])) {
        ...
        if ($result->num_rows > 0) {
            $_SESSION['aadhar'] = $aadhar;  // Store Aadhaar number in session
    ...
    <script>
        window.onload = function () {
            ...
            captureButton.addEventListener('click', async () => {
                ...
                // Send the image and Aadhaar number to save it on the server via AJAX
                fetch('set_session.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        imageData: imageDataUrl,
                        aadhar: '<?php echo $_SESSION['aadhar']; ?>' // Pass the Aadhaar number from the session
                    })
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        // Redirect to voteteam.php after successful session storage
